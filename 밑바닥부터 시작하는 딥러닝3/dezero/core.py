...
class Variable:
    ...
    def backward(self, retain_grad=False, create_graph=False): # create_graph로 gradient 계산과정을 계산그래프로 남길지 결정
        if self.grad is None:
            # self.grad = np.ones_like(self.data)
            self.grad = Variable(np.ones_like(self.data)) # 미분값(ndarray) 저장에서 Variable 인스턴스를 저장하도록 변경
        ...
        while funcs:
            ...
            with using_config('enable_backprop', create_graph):
                # backward 내부 연산에서 Function.__call__()이 실행될 때 Config.enable_backprop 값을 참조하여 역전파 그래프 생성(create_graph) 여부 결정
                ...
                for x, gx in zip(f.inputs, gxs):
                    if x.grad is None:
                        ...
                        x.grad = x.grad + gx # 이 계산도 계산 그래프 생성 여부 결정
...
class Pow(Function):
    ...
    def backward(self, gy):
        x, = self.inputs # self.inputs = [as_variable(x) for x in inputs]
        c = self.c
        gx = c * x ** (c - 1) * gy
        return gx
